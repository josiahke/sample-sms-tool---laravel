...
class AfricasTalkingGateway
{
    ...
    //Messaging methods
    public function sendMessage($to_, $message_, $from_ = 'MOOKH', $bulkSMSMode_ = 1, Array $options_ = [])
    {
        if (strlen($to_) == 0 || strlen($message_) == 0) {
            //throw new Exception('Please supply both to and message parameters');
            Log::alert('Please supply both to and message parameters');
        }
        ...
        if (count($options_) > 0) {
            ...
            foreach ($options_ as $key => $value) {
                if (in_array($key, $allowedKeys) && strlen($value) > 0) {
                    ...
                    Log::alert("Invalid key in options array: [$key]");
                }
            }
        }
...
//            throw new Exception('Please supply phoneNumber, shortCode and keyword');
            Log::alert('Please supply phoneNumber, shortCode and keyword');
        }
    ...
    public function call($from_, $to_)
    {
        if (strlen($from_) == 0 || strlen($to_) == 0) {
            ...
            Log::alert('Please supply both from and to parameters');
        }
        ...
        if (($responseObject = json_decode($this->_responseBody)) !== null) {
            if (strtoupper(trim($responseObject->errorMessage)) == "NONE") {
                ...
                Log::alert($responseObject->errorMessage);
            }
    ...
    private function setCurlOpts(&$curlHandle_)
    {
        ...
        curl_setopt($curlHandle_, CURLOPT_HTTPHEADER, ['Accept: application/json',
            'apikey: ' . $this->_apiKey]);
    }
}
